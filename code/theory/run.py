...
DEFAULT_SAMPLE_SIZES = (100, 200, 500, 1_000)
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=repo_root / "knowledge" / "theory" / "results",
    )
    return parser
...
def main() -> None:
    ...
    if args.replications <= 0 or args.batch_size <= 0:
        raise ValueError("replications and batch-size must be positive")
    if any(value < 10 for value in args.sample_sizes):
